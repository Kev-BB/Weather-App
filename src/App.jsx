...
export default function App() {
  const [currentDay, setCurrentDay] = useState(0); // Initialize with Pazartesi
  ...
  const { day, condition, lowTemp, highTemp } = weatherData[currentDay];

  return (
    <div className={`app-container ${condition.toLowerCase()}-background`}>
      <div className='weather-container'>
        <div className='icon'>{condition === 'Güneşli' ? '☀️' : condition === 'Yağmurlu' ? '🌧️' : '❄️'}</div>
